...
pub fn delete_crate(conn: &Connection, name: &str) -> Result<(), Error> {
    ...
    if let Some(s3) = s3_client() {
        let mut backend = S3Backend::new(s3, S3_BUCKET_NAME);
...
fn delete_from_s3(s3: &mut S3Backend, name: &str) -> Result<(), Error> {
    for prefix in STORAGE_PATHS_TO_DELETE {
...
fn delete_prefix_from_s3(s3: &mut S3Backend, name: &str) -> Result<(), Error> {
